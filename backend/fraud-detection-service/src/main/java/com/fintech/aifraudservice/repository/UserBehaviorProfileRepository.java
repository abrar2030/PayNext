...
public interface UserBehaviorProfileRepository extends JpaRepository<UserBehaviorProfile, Long> {
  ...
  @Query(
      "SELECT ubp FROM UserBehaviorProfile ubp WHERE ubp.lastAnalysisDate < :cutoffDate OR"
          + " ubp.lastAnalysisDate IS NULL")
  List<UserBehaviorProfile> findProfilesNeedingUpdate(
  ...
  @Query(
      "SELECT ubp FROM UserBehaviorProfile ubp WHERE ubp.fraudIncidents > :threshold ORDER BY"
          + " ubp.fraudIncidents DESC")
