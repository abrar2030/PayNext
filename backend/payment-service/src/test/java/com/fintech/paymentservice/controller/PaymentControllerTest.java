...
class PaymentControllerTest {
    ...
    void getPaymentById_whenPaymentDoesNotExist_shouldReturnNotFound() throws Exception {
        when(paymentService.getPaymentById(anyLong())).thenThrow(new RuntimeException("Payment not found")); // Or return null/Optional.empty based on service impl
    ...
    void getPaymentsByUserId_shouldReturnPaymentList() throws Exception {
        ...
        mockMvc.perform(get("/api/payments/user/{userId}", 100L)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
