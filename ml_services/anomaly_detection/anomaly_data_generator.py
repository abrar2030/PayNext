...
def generate_synthetic_transaction_data(num_transactions=10000, num_users=100, anomaly_ratio=0.02):
    np.random.seed(42)

    users = [f'user_{i:03d}' for i in range(num_users)]
    merchants = ['GroceryStoreA', 'RestaurantB', 'OnlineShopC', 'UtilityCoD', 'GasStationE', 'CoffeeShopF', 'BookStoreG', 'TravelAgencyH']
    ...
    for i in range(num_transactions):
        ...
        if np.random.rand() < anomaly_ratio:
            is_anomaly = True
            anomaly_type = np.random.choice(['high_value', 'unusual_time', 'unusual_merchant', 'unusual_frequency'])

            if anomaly_type == 'high_value':
                amount = np.random.uniform(2000, 10000) # Very high value transaction
            elif anomaly_type == 'unusual_time':
                timestamp = timestamp.replace(hour=np.random.choice([2,3,4]), minute=np.random.randint(0, 60)) # Middle of the night
            elif anomaly_type == 'unusual_merchant':
                merchant = np.random.choice(['DarkWebVendor', 'OffshoreService', 'RareCollectibleStore']) # Unusual merchant
    ...
    df = pd.DataFrame(data, columns=['user_id', 'timestamp', 'amount', 'merchant', 'transaction_type', 'location', 'is_anomaly'])
    return df

if __name__ == '__main__':
    df = generate_synthetic_transaction_data(num_transactions=100000, num_users=500, anomaly_ratio=0.01)
