...
transaction_type_encoder_path = os.path.join(model_dir, "transaction_type_encoder.joblib")
...
try:
    ...
    print(f"Fraud detection model or encoder file not found: {e}. Please train the models first.")
...
async def predict_fraud(transaction: FraudPredictionInput):
    if any(m is None for m in [fraud_model_rf, fraud_model_if, fraud_model_ae, fraud_scaler, fraud_model_features,
                                location_encoder, merchant_encoder, transaction_type_encoder, user_id_encoder]):
        raise HTTPException(status_code=500, detail="Fraud detection models or encoders not loaded. Please train the models.")
    ...
    try:
        ...
        transaction_type_encoded = get_encoded_value(transaction_type_encoder, transaction.transaction_type)
