...
import { ArrowUpRight, ArrowDownLeft, QrCode, Send, Landmark } from 'lucide-react'; // Import icons
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-3 gap-4">
          ...
          <Link href="/request" passHref>
            <Button variant="outline" className="flex flex-col items-center justify-center h-24 w-full">
              <Landmark className="h-6 w-6 mb-1 text-green-500" /> {/* Changed icon for Request */}
            ...
            <ul className="space-y-4">
              {transactions.map((tx) => (
                <li key={tx.id} className="flex items-center space-x-4">
                  ...
                  <p className={`font-medium text-sm ${tx.amount < 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
