...
jest.mock('../../../services/RequestService', () => ({
  submitPaymentRequest: jest.fn().mockResolvedValue({ success: true, requestId: 'req_123' }),
}));
...
describe('Mobile RequestPage', () => {
  test('renders request form elements', () => {
    ...
    expect(screen.getByRole('button', { name: /generate request link/i })).toBeInTheDocument(); // Or similar button text
  });
  ...
  test('allows user to input request details', () => {
    ...
    fireEvent.change(descriptionInput, { target: { value: 'Payment for lunch' } });
